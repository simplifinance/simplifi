...
import { DRAWERWIDTH, ROUTE_ENUM, flexEnd, flexSpread, flexStart } from '@/constants';
...
export default function App(props: AppProps) {
  ...
  return (
    <React.Fragment>
      ...
      <Stack className={`bg-green1 py-2 px-4 h-full ${location === '/dashboard'? 'space-y-24' : 'space-y-14'} '`}>
        <nav style={{ width: `calc(100% - ${DRAWERWIDTH}px)`,  marginLeft: `${DRAWERWIDTH}px`, borderBottom: '1px solid rgba(255, 255, 255, 0.2)', }} className={`fixed py-2 px-4  ${flexSpread}`}>
          <h3 className='text-lg font-semibold text-orangec'>{ `${location[1]?.toUpperCase()}${location?.substring(2, location.length)}` }</h3>
          <div className={`${flexSpread} w-[60%] gap-6`}>
            <div className='w-[30%] '>
            ...
            <div className={`${flexSpread} gap-4 mr-8 w-[70%]`}>
...
const setIcon = (selector: string, isActive: boolean) => {
  ...
  const icons = Array.from([
    ...
    // 'digdao': 
      <svg key={3} width="20" height="20" viewBox="0 0 20 20" fill={`${isActive? "rgba(0, 0, 0, 0.7)" : "rgba(0, 0, 0, 0.2)"}`} xmlns="http://www.w3.org/2000/svg">
