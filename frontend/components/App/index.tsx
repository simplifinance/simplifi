...
export default function App(props: AppProps) {
  ...
  const handleModalClose = () => setShowSideMenu(false);
  ...
  return (
    <div className="flex justify-between border-none ">
      <SideDrawer { ...{ setIcon, setParentActive, parentLinkActive } } />
      <MobileSideDrawer { ...{ setIcon, setParentActive, parentLinkActive, modalOpen:showSideMenu, handleModalClose} } />
      <Box width={{xs: "100%", lg: `calc(100% - ${DRAWERWIDTH}px)`}} className={`${location === '/dashboard'? 'space-y-' : 'space-y-'} h-screen`}>
        <nav className='p-4 border-b border-b-gray-600/30 flex justify-between items-center'>
          <div className='hidden lg:inline-block w-1/4'>
            <h1 className='text-xl font-black text-orangec'>{ `${location[1]?.toUpperCase()}${location?.substring(2, location.length)}` }</h1>
          </div>
          <div className={`flex justify-between lg:justify-end w-full md:px-4 lg:gap-4`}>
            {
              !showSideMenu?
                <button className='lg:hidden hover:text-orange-400' onClick={() => setShowSideMenu(!showSideMenu)}>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 text-white1/60 border border-gray-600/40 rounded active:ring-1 ">
        ...
        <MotionDivWrap className={`w-full overflow-auto mt-10 lg:mt-12 px-4 pb-8 lg:pb-0`} >
          <Outlet />
        </MotionDivWrap>
      </Box>
    </div>
