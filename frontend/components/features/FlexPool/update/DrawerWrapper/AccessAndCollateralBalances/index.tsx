...
import { readUserDataConfig, } from "../readContractConfig";
...
export default function AccessAndCollateralBalances({ handleCloseDrawer, formatted_bank, } : AccessAndCollateralBalanceProps) {
        ...
        <Stack className="bg-gray1 p-4 space-y-2 rounded-lg text-orange-400 font-normal text-sm">
            ...
            <div className="p-2 border border-green1 rounded-lg text-orange-200 text-xs space-y-1">
                <div className={`${flexSpread}`}>
                    <h1>In Use</h1>
                    {
                        isPending || !data? <Spinner color="#fed7aa" /> : <h1>{`${toBN(formatEther(data?.collateral.balance || 0n)).decimalPlaces(2).toString()} XFI`}</h1>
                    }
                </div>
                <div className={`${flexSpread}`}>
                    <h1>Withdrawable</h1>
                    {
                        isPending || !data? <Spinner color="#fed7aa" /> : <h1>{`${toBN(formatEther(data?.collateral.withdrawable || 0n)).decimalPlaces(2).toString()} XFI`}</h1>
                    }
                </div>
            </div>
            <ButtonTemplate
