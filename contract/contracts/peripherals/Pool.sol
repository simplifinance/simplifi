...
import { Contributor, Common, ErrorLib, Utils, IRoleBase, IERC20, IPoint } from "./Contributor.sol";
import { SafeManager } from "./SafeManager.sol";
import { ISafe } from "../apis/ISafe.sol";

contract Pool is SafeManager, Contributor {
    using Utils for *;
    using ErrorLib for *;
    ...
    function _createPool(
        ...
        pool = _updatePool(unitId, Common.UpdatePoolData(unit, unitId, recordId, maxQuorum, colCoverage, colAsset, durationInHours, users[0]));  
    ...
    function _addUserToPool(
        ...
        for(uint i = 0; i < users.length; i++) {
            Common.Contributor memory profile; 
            uint8 slot;
            if(i == 0) (profile, slot) = _initializeContributor(_pool, unit, users[i], true, true, true);
            else (profile, slot) = _initializeContributor(_pool, unit, users[i], false, true, false);
    ...
    */
    function _joinAPool(
        ...
        if(_pool.router == Common.Router.PERMISSIONED) {
            ...
            (profile, slot) = _initializeContributor(_pool, unit, user, false, true, true);
        }
        _setContributor(profile, _pool.big.recordId, slot, false);
        unchecked {
            _pool.big.currentPool += unit;
    ...
    */
    function _completeAddUser(address user, Common.Pool memory pool) internal {
        (, uint) = _checkAndWithdrawAllowance(IERC20(baseAsset), user, pool.addrs.safe, pool.big.unit);
