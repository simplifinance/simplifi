...
describe("Providers", function () {
  ...
  describe("Contributors are able to finance unit contribution through external providers", function () {
    it("Signer should borrow. This should successfully add user to existing flexpool if it exist and they meet criteria", async function () {
      const {
        baseAsset,
        rate,
        collateralAsset,
      ...
      const { pool, } = await borrow({amount, contract:providers, flexpool, providersSlots: [prov.profile.slot], signer: signer2, signerAddr: signer2Addr});
