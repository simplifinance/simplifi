...
abstract contracts Epoches is PastEpoches {
    ...
    */
    function _isUnitActive(uint unit) internal pure returns(bool result){
    ...
    function _getRecordId(uint unit) internal view returns(uint _recordId) {
        _recordId = _getPool(unit).big.recordId;
    }

    // Generate unit Id and record Id
    function _generateIds(uint256 unit) internal view returns(uint unitId, uint recordId) {
        epoches.increment();
        _unitId = epoches.current();
        indexes[unit] = _unitId;
