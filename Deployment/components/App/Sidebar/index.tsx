...
export default function Sidebar({ setParentActive, parentLinkActive } : SidebarProps) {
  ...
  return(
    <aside className={`${!showSidebar? 'hidden' : 'flex'} md:flex z-50 md:p-[22px]`}>
      <div className="w-full md:rounded-[56px] min-h-[fit-content] animateSidebar">
        {
          DRAWER_CONTENT.map(({collapsible, parentPath, icon, parentTitle, displayChevron, children }) => (
            <Collapsible 
              key={parentTitle}
                {
                  ...{
...
const DRAWER_CONTENT = [
  ...
  },
  {
    parentTitle: 'Flex',
    collapsible: true,
    parentPath: ROUTE_ENUM.FLEXPOOL,
    displayChevron: true,
    children: 
