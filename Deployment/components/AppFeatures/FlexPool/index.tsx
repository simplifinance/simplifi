      ...
      <div className="flex justify-between items-center gap-2">
        <div className={`${flexSpread} gap-2`}>
          ...
          <div className={`hidden md:flex items-center gap-2 w-[fit-content] text-xs `}>
            <Button disabled={!isPastEpoches}  variant={'outline'} onClick={() => setEpochType(false)} className={` text-green1/70 dark:text-white2 ${isPastEpoches? 'dark:bg-gray1 animate-pulse hover:text-orangec' : 'dark:bg-green1 '} p-3 rounded-xl`}>
              Active Pools
            </Button>
            <Button disabled={isPastEpoches} variant={'outline'}  onClick={() => setEpochType(true)} className={` text-green1/70 dark:text-white2 ${!isPastEpoches? 'dark:bg-gray1 animate-pulse hover:text-orangec' : 'dark:bg-green1 '} p-3 rounded-xl`}>
              Past Pools
            </Button>
          </div>
          {/* <USDBalances /> */}
        ...
        <div className={`${flexSpread} relative gap-2`}>
          <Button variant={'outline'}  onClick={() => setActivepath('')} className={`text-green1/70 dark:text-white2 dark:bg-gray1 dark:hover:text-orangec p-3 rounded-xl`}>
            Back
          </Button>
          <Button variant={'outline'} onClick={handleCreatePool} className={`text-green1/70 dark:text-white2 dark:bg-gray1 dark:hover:text-orangec p-3 rounded-xl`}>
            <h3>Create</h3>
            <Tooltip title="New Pool">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5 md:size-6 text-green1/70 dark:text-orange-300 ">
...
//               handleCreatePool={openDisplayForm}
//             >
